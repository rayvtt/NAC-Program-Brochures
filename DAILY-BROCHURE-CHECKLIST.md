...
| 8 | **English version actually displays on EN click** | Simulates `setLang('en')` against the live page DOM and checks that **no Vietnamese remnants are left** visible. Catches: DOM strings not in arrays, empty EN slots, whitespace/entity drift, missing selectors. Failure = the user sees mixed VN+EN after clicking English. Run via `tools/simulate_en_render.py <alias>` for a per-string breakdown. |
