...
Mirrors the design used on the Property Hub listing PDPs (e.g. the Nobu
Da Nang listing) — a dark glassy floating pill at the bottom-center,
gold-glow shadow, refined SVG icons. Icon-only when collapsed; expands
to icon + label on hover (desktop) or tap (touch).

Brochure pill links: WhatsApp · Property Hub · Blog.
...
Idempotency:
  - Removes the legacy first-pass .nac-mobile-cta block (if present)
    before installing the new pill, so re-runs after a design change
    end up clean rather than stacked.
...
# Legacy v1 CTA bar — detect and strip before installing v2 pill.
# Matches the whole block: comment header + .nac-mobile-cta rule +
# @media (max-width: 720px) {...} block, ending at the first un-indented
# closing brace (which terminates the @media block).
LEGACY_CSS_RE = re.compile(
    r'\n?/\*\s*=+\s*\n\s*MOBILE CTA BAR.*?\n}\n',
    re.DOTALL,
)
# Catches a free-standing .nac-mobile-cta block (no comment header) and
# the surrounding @media. Used as a fallback when only the comment was
# stripped on a previous run.
LEGACY_CSS_NOHEAD_RE = re.compile(
    r'\n?\.nac-mobile-cta\s*\{[^}]*\}\s*\n\s*@media \(max-width: 720px\) \{.*?\n}\n',
    re.DOTALL,
)
LEGACY_HTML_RE = re.compile(
...
CSS_BLOCK = '''
/* ============================================================
   NAC TOOLS PILL — floating bottom-center pill (Nobu PDP style)
   Mobile-only on brochures. Auto-collapses to icon-only; expands
   on hover (desktop) or tap (touch). Dark glass + gold glow.
   ============================================================ */
...
@media (max-width: 720px) {
  ...
  .nac-tools.is-open .nac-tool { gap:.4rem; padding:.5rem .9rem; }
  .nac-tools.is-open .nac-tool-txt { max-width:100px; opacity:1; transform:translateX(0); }

  /* Color-coded buttons */
  .nac-tool--wa    { color:#25D366; }
  .nac-tool--wa:hover, .nac-tool--wa:active   { color:#3ee87b; background:rgba(37,211,102,.18); }
  .nac-tool--hub   { color:#f5d07a; }
  .nac-tool--hub:hover, .nac-tool--hub:active { color:#ffe49a; background:rgba(232,191,114,.18); }
  .nac-tool--blog  { color:#e89a6a; }
  .nac-tool--blog:hover, .nac-tool--blog:active { color:#ffb380; background:rgba(217,124,68,.20); }

  /* Push float-toc-btn up so the pill doesn't crowd it */
  .float-toc-btn { bottom: 84px !important; }

  /* Give the body bottom breathing room so the last section doesn't
     sit under the pill on scroll-end */
  body { padding-bottom: 80px; }
}
'''

HTML_BLOCK = '''
<!-- NAC TOOLS PILL — floating bottom CTA (mobile only; mirrors PH PDP style; installed by tools/install_mobile_cta.py) -->
<div class="nac-tools">
  ...
  <a href="https://nomadassetcollective.com/property-hub/" target="_blank" rel="noopener" class="nac-tool nac-tool--hub" aria-label="Property Hub">
    <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 17l6-6 4 4 8-8M14 7h7v7"/></svg>
...
def patch_one(path):
    ...
    # 0. Strip legacy v1 CTA bar if present (we want a single source of truth).
    new = LEGACY_CSS_RE.sub('', doc)
    new = LEGACY_CSS_NOHEAD_RE.sub('', new)
    new = LEGACY_HTML_RE.sub('', new)
    if new != doc:
        did.append('removed v1')
        doc = new

    # 1. CSS — inject before </style>
    if 'NAC TOOLS PILL — floating bottom-center pill' not in doc:
        if '</style>' not in doc:
